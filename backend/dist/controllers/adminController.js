...
exports.autoLockPenaltyDebtUsers = exports.autoLockOverdueUsers = exports.getUserStatistics = exports.getBookAndBorrowStatistics = exports.deleteUser = exports.getUserById = exports.toggleUserStatus = exports.updateUserRole = exports.getAllUsers = void 0;
...
const getAllUsers = async (req, res) => {
    try {
        const { page = 1, limit = 10, role, status, isActive, search, sort = 'createdAt', order = 'desc' } = req.query;
...
const updateUserRole = async (req, res) => {
    try {
        ...
        // Update user role
        const updatedUser = await User_1.default.findByIdAndUpdate(id, { role }, { new: true }).select('-passwordHash -verificationToken -resetPasswordToken');
        res.status(200).json({
            success: true,
...
const toggleUserStatus = async (req, res) => {
    try {
        ...
        const updatedUser = await User_1.default.findByIdAndUpdate(id, { isActive: newStatus }, { new: true }).select('-passwordHash -verificationToken -resetPasswordToken');
        res.status(200).json({
            success: true,
...
const getBookAndBorrowStatistics = async (req, res) => {
    try {
        ...
        const activeBooks = await Book_1.default.countDocuments({ isActive: true });
        const inactiveBooks = await Book_1.default.countDocuments({ isActive: false });
        // Thống kê theo status
        const booksByStatus = await Book_1.default.aggregate([
            {
                $group: {
                    _id: '$status',
                    count: { $sum: 1 }
        ...
        const premiumBooks = await Book_1.default.countDocuments({ isPremium: true, isActive: true });
        const newReleaseBooks = await Book_1.default.countDocuments({ isNewRelease: true, isActive: true });
        // Thống kê tổng stock và available
        const stockStats = await Book_1.default.aggregate([
            {
                $group: {
                    _id: null,
                    totalStock: { $sum: '$stock' },
        ...
        const newBooks = await Book_1.default.countDocuments(newBooksQuery);
        // Top 10 sách được mượn nhiều nhất
        const topBorrowedBooks = await Borrow_1.default.aggregate([
            {
                $group: {
                    _id: '$book',
                    borrowCount: { $sum: 1 },
        ...
        const topRatedBooks = await Book_1.default.find({ isActive: true, reviewCount: { $gt: 0 } })
            .sort({ rating: -1, reviewCount: -1 })
            .limit(10)
            .select('title category rating reviewCount stock available isPremium');
        // Sách sắp hết (available < 10% stock)
        const lowStockBooks = await Book_1.default.aggregate([
            {
                $match: {
                    isActive: true,
                    stock: { $gt: 0 }
        ...
        const booksByDay = await Book_1.default.aggregate([
            {
                $match: {
        ...
        const booksByMonth = await Book_1.default.aggregate([
            {
                $match: {
        ...
        // Tổng số phiếu mượn
        const totalBorrows = await Borrow_1.default.countDocuments();
        // Thống kê theo status
        ...
        const newBorrowsQuery = {};
        if (startDate) {
            newBorrowsQuery.borrowDate = { $gte: startDate };
        }
        const newBorrows = await Borrow_1.default.countDocuments(newBorrowsQuery);
        // Thống kê mượn theo ngày (7 ngày gần nhất)
        const borrowsByDay = await Borrow_1.default.aggregate([
            {
                $match: {
        ...
        const borrowsByMonth = await Borrow_1.default.aggregate([
            {
                $match: {
        ...
        // Thống kê phí phạt
        const lateFeeStats = await Borrow_1.default.aggregate([
            {
                $group: {
                    _id: null,
        ...
        // Thống kê gia hạn
        const renewalStats = await Borrow_1.default.aggregate([
            {
                $group: {
                    _id: null,
        ...
        // Thống kê BookCopy
        const totalBookCopies = await BookCopy_1.default.countDocuments();
        ...
        const borrowTypeStats = {};
        borrowsByType.forEach((item) => {
            borrowTypeStats[item._id] = item.count;
        });
        const copyStatusStats = {};
        bookCopiesByStatus.forEach((item) => {
...
exports.getBookAndBorrowStatistics = getBookAndBorrowStatistics;
...
const getUserStatistics = async (req, res) => {
    try {
        ...
        // Thống kê theo status
        const usersByStatus = await User_1.default.aggregate([
            {
                $group: {
                    _id: '$status',
                    count: { $sum: 1 }
        ...
        const usersWithMembership = await User_1.default.countDocuments({
        ...
        const newUsers = await User_1.default.countDocuments(newUsersQuery);
        // Thống kê người dùng đăng ký theo ngày (7 ngày gần nhất)
        const last7Days = new Date(now);
        last7Days.setDate(now.getDate() - 7);
        const usersByDay = await User_1.default.aggregate([
            {
                $match: {
                    createdAt: { $gte: last7Days }
                }
        ...
        const totalBorrows = await Borrow_1.default.countDocuments();
        const activeBorrows = await Borrow_1.default.countDocuments({
            status: { $in: ['Borrowed', 'Overdue'] }
        });
        const returnedBorrows = await Borrow_1.default.countDocuments({ status: 'Returned' });
        // Top 10 người dùng mượn nhiều nhất
        const topBorrowers = await Borrow_1.default.aggregate([
            {
                $group: {
                    _id: '$user',
                    totalBorrows: { $sum: 1 },
        ...
        const activeSubscriptions = await MembershipSubscription_1.default.countDocuments({
            status: 'Active'
        });
        const expiredSubscriptions = await MembershipSubscription_1.default.countDocuments({
            status: 'Expired'
        });
        // Thống kê người dùng có thể mượn sách
        const usersCanBorrow = await User_1.default.countDocuments({ canBorrow: true });
        const usersCannotBorrow = await User_1.default.countDocuments({ canBorrow: false });
        // Thống kê tổng chi tiêu
        const totalSpentStats = await User_1.default.aggregate([
            {
                $group: {
                    _id: null,
        ...
        // Thống kê ví
        const walletStats = await User_1.default.aggregate([
            {
                $group: {
                    _id: null,
...
const autoLockOverdueUsers = async (req, res) => {
    try {
        const result = await (0, violationService_1.autoLockBorrowingPermissionForOverdue)();
        res.status(200).json({
            success: true,
...
exports.autoLockOverdueUsers = autoLockOverdueUsers;
// POST /admin/users/auto-lock-penalty-debt - Tự động khóa quyền mượn khi nợ phạt vượt mức
const autoLockPenaltyDebtUsers = async (req, res) => {
    try {
        const result = await (0, violationService_1.autoLockBorrowingPermissionForPenaltyDebt)();
        res.status(200).json({
            success: true,
...
exports.autoLockPenaltyDebtUsers = autoLockPenaltyDebtUsers;
