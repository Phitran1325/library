...
exports.getPublicBookById = exports.getPublicBooks = exports.listBookEbooks = exports.downloadBookEbook = exports.uploadBookEbook = exports.getBooksByVolume = exports.getNewReleases = exports.updateBookStock = exports.toggleBookStatus = exports.deleteBook = exports.updateBook = exports.createBook = exports.getBookById = exports.getAllBooks = void 0;
...
const getAllBooks = async (req, res) => {
    try {
        const { page = 1, limit = 10, category, categoryId, status, isActive, isNewRelease, author, // legacy alias for authorId
        authorId, authorName, publisher, // legacy alias for publisherId
        publisherId, title, search, sort = 'createdAt', order = 'desc' } = req.query;
        ...
        if (authorName) {
            const matchedAuthors = await Author_1.default.find({ name: { $regex: String(authorName), $options: 'i' }, isActive: true }).select('_id');
        ...
        const books = await Book_1.default.find(query)
            ...
            .skip(skip)
            .limit(Number(limit));
        const total = await Book_1.default.countDocuments(query);
        res.status(200).json({
            success: true,
            data: {
                books,
...
const createBook = async (req, res) => {
    try {
        const { title, isbn, description, coverImage, pages, publicationYear, publishedDate, language, category, categoryId, price, rentalPrice, stock, volume, isNewRelease, authorId, publisherId, tags } = req.body;
        ...
        const validAuthor = await Author_1.default.findOne({ _id: authorId, isActive: true });
        if (!validAuthor) {
            return res.status(400).json({
                success: false,
        ...
        const validPublisher = await Publisher_1.default.findOne({ _id: publisherId, isActive: true });
        if (!validPublisher) {
            return res.status(400).json({
                success: false,
...
const updateBook = async (req, res) => {
    try {
        const { id } = req.params;
        const { title, isbn, description, coverImage, pages, publicationYear, publishedDate, language, category, categoryId, price, stock, volume, isNewRelease, authors, publisher, tags, status, isActive } = req.body;
        ...
        if (authors && authors.length > 0) {
            const validAuthors = await Author_1.default.find({ _id: { $in: authors }, isActive: true });
        ...
        if (publisher) {
            const validPublisher = await Publisher_1.default.findOne({ _id: publisher, isActive: true });
            if (!validPublisher) {
                return res.status(400).json({
                    success: false,
                    message: 'Nhà xuất bản không tồn tại hoặc đã bị vô hiệu hóa'
        ...
        const updatedBook = await Book_1.default.findByIdAndUpdate(id, {
            title,
            isbn,
            description,
            coverImage,
...
const toggleBookStatus = async (req, res) => {
    try {
        ...
        const updatedBook = await Book_1.default.findByIdAndUpdate(id, { isActive: newStatus }, { new: true })
            .populate('authors', 'name nationality')
            .populate('publisher', 'name');
        res.status(200).json({
            success: true,
...
const updateBookStock = async (req, res) => {
    try {
        ...
        const updatedBook = await Book_1.default.findByIdAndUpdate(id, updateData, { new: true, runValidators: true })
            .populate('authors', 'name nationality')
            .populate('publisher', 'name');
        res.status(200).json({
            success: true,
...
const getBooksByVolume = async (req, res) => {
    try {
...
exports.getBooksByVolume = getBooksByVolume;
const mimeToEbookFormat = {
    'application/pdf': 'PDF',
    'application/epub+zip': 'EPUB',
};
const isMembershipActive = (user, membershipPlan) => {
...
const membershipHasEbookFeature = (membershipPlan) => {
    if (!membershipPlan)
        return false;
...
const canDownloadEbook = (user, book) => {
    if (!user) {
 ...
 */
const uploadBookEbook = async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const uploadResult = await (0, uploadService_1.uploadEbookBuffer)(req.file.buffer, {
            format,
        ...
        book.digitalFiles.push(digitalFile);
        await book.save();
...
const downloadBookEbook = async (req, res) => {
    try {
        ...
        if (!mongoose_1.default.Types.ObjectId.isValid(bookId) || !mongoose_1.default.Types.ObjectId.isValid(fileId)) {
            return res.status(400).json({
                success: false,
        ...
        const [book, user] = await Promise.all([
            ...
            User_1.default.findById(userId).populate('membershipPlanId'),
        ]);
        ...
        const downloadInfo = (0, uploadService_1.generateEbookDownloadUrl)(digitalFile.publicId, digitalFile.format || 'pdf');
        return res.status(200).json({
            success: true,
...
exports.downloadBookEbook = downloadBookEbook;
/**
 * GET /api/books/:id/ebooks
 * Danh sách ebook của một sách (yêu cầu auth)
 */
const listBookEbooks = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.user?.userId;
        if (!userId) {
        ...
        const [book, user] = await Promise.all([
            Book_1.default.findById(id).select('title isPremium digitalFiles'),
            User_1.default.findById(userId).populate('membershipPlanId'),
        ]);
        if (!book || !book.digitalFiles || book.digitalFiles.length === 0) {
            return res.status(404).json({
...
exports.listBookEbooks = listBookEbooks;
...
const getPublicBooks = async (req, res) => {
    try {
        const { page = 1, limit = 10, category, categoryId, isNewRelease, authorId, authorName, publisherId, title, search, hasEbook, sort = 'createdAt', order = 'desc' } = req.query;
        const skip = ((Number(page) - 1) * Number(limit));
        ...
        // Filter by authorName (convert to authorId list)
        if (authorName) {
            const matchedAuthors = await Author_1.default.find({
...
const getPublicBookById = async (req, res) => {
    try {
        ...
        let book;
        if (mongoose_1.default.Types.ObjectId.isValid(id)) {
            book = await Book_1.default.findOne({
                _id: id,
                isActive: true,
                status: 'available'
            })
...
exports.getPublicBookById = getPublicBookById;
