...
exports.verifyPayOSSignature = verifyPayOSSignature;
...
async function createPayOSPaymentLink(params) {
    const { userId, type, planId, bookId, rentalDays, amount, description, expiresInMinutes = 30, metadata = {} } = params;
    ...
    if (!clientId || !apiKey || enableMockMode) {
        ...
        const payment = await Payment_1.default.create({
            ...
            book: type === 'Rental' ? bookId : undefined,
    ...
    try {
        ...
        if (checksumKey) {
            ...
            body.signature = crypto_1.default.createHmac('sha256', checksumKey).update(raw).digest('hex');
        }
...
function verifyPayOSSignature(payload, signature) {
    ...
    const sigFull = crypto_1.default.createHmac('sha256', secret).update(rawFull).digest('hex');
    if (sigFull === signature)
        return true;
    if (payload?.data) {
        const rawData = JSON.stringify(payload.data);
        const sigData = crypto_1.default.createHmac('sha256', secret).update(rawData).digest('hex');
