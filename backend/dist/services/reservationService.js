...
exports.assignReservationToUser = assignReservationToUser;
exports.approveReservation = approveReservation;
exports.cancelReservation = cancelReservation;
exports.fulfillReservation = fulfillReservation;
exports.updateQueuePositions = updateQueuePositions;
exports.assignNextReservation = assignNextReservation;
 ...
 */
function getObjectIdString(field) {
    if (!field) {
        throw new Error('Trường dữ liệu không tồn tại');
    }
    // Nếu là ObjectId hoặc string
    if (typeof field === 'string' || mongoose_1.default.Types.ObjectId.isValid(field)) {
 ...
 */
async function validateReservation(userId, bookId) {
    const errors = [];
    ...
    if (currentReservations >= rules.maxReservations) {
        errors.push(`Bạn đã đạt tối đa ${rules.maxReservations} lượt đặt trước. ` +
            `Vui lòng hủy một đặt trước trước khi tạo mới.`);
    }
    // 4. Kiểm tra nợ phí phạt quá lớn
    if (user.debt > 500000) { // MAX_DEBT_ALLOWED từ debtService
        errors.push(`Bạn có nợ phí phạt ${user.debt.toLocaleString('vi-VN')} VNĐ. ` +
...
async function createReservation(userId, bookId) {
    ...
    const pendingCountBefore = await Reservation_1.default.countDocuments({
        book: bookId,
        status: 'Pending'
    });
    ...
    const updatedReservation = await Reservation_1.default.findById(reservation._id)
        .populate('book')
        .populate('user');
    ...
    const bookAfterReload = updatedReservation.book;
    if (bookAfterReload.available > 0) {
        await assignReservationToUser(String(updatedReservation._id));
        // Reload lại sau khi assign
        const finalReservation = await Reservation_1.default.findById(updatedReservation._id);
        return finalReservation || updatedReservation;
    }
    // Nếu sách không có sẵn, gửi email xác nhận với queuePosition đã được cập nhật
    (0, emailService_1.sendReservationConfirmationEmail)(user.email, bookAfterReload.title, updatedReservation.queuePosition || 1, expiresAt).catch(err => console.error('Không thể gửi email xác nhận đặt trước:', err));
 ...
 */
async function assignReservationToUser(reservationId) {
    ...
    if (book.available <= 0) {
        ...
        const queuePosition = await Reservation_1.default.countDocuments({
            book: book._id,
            status: 'Pending',
    ...
    const membershipType = membershipPlan?.name || borrowingConstants_1.MembershipType.STANDARD;
    const rules = (0, borrowingConstants_1.getBorrowingRules)(membershipType);
    reservation.expiresAt = new Date(Date.now() + rules.reservationHoldHours * 60 * 60 * 1000);
    await reservation.save();
    ...
    (0, emailService_1.sendReservationAvailableEmail)(user.email, book.title, reservation.expiresAt).catch(err => console.error('Không thể gửi email thông báo sách có sẵn:', err));
    return reservation;
}
/**
 * Duyệt reservation bởi thủ thư
 */
async function approveReservation(reservationId, librarianId) {
    const reservation = await Reservation_1.default.findById(reservationId)
        .populate('user')
        .populate('book');
    if (!reservation) {
    ...
    if (book.available > 0) {
        ...
        const finalReservation = await Reservation_1.default.findById(reservationId)
            .populate('user')
            .populate('book');
        if (!finalReservation) {
    ...
    await updateQueuePositions(book._id.toString());
    // Reload reservation để lấy queuePosition đã được cập nhật
    const updatedReservation = await Reservation_1.default.findById(reservationId)
        .populate('user')
        .populate('book');
    if (!updatedReservation) {
        throw new Error('Không thể tải lại đặt trước sau khi cập nhật');
...
async function cancelReservation(reservationId, userId) {
    ...
    const reservationForCheck = await Reservation_1.default.findById(reservationId);
    ...
    if (userId) {
        ...
        if (mongoose_1.default.Types.ObjectId.isValid(reservationUserId) &&
            mongoose_1.default.Types.ObjectId.isValid(requestUserId)) {
            const reservationObjId = new mongoose_1.default.Types.ObjectId(reservationUserId);
            const requestObjId = new mongoose_1.default.Types.ObjectId(requestUserId);
...
async function fulfillReservation(reservationId) {
    ...
    const borrow = await (0, borrowService_1.createBorrow)(user._id.toString(), book._id.toString());
 ...
 */
async function updateQueuePositions(bookId) {
    const now = new Date();
    // Chỉ lấy reservation Pending và chưa hết hạn
    const pendingReservations = await Reservation_1.default.find({
        book: bookId,
        status: 'Pending',
        $or: [
 ...
 */
async function assignNextReservation(bookId) {
    // Tìm reservation Pending đầu tiên theo thứ tự FIFO
    const nextReservation = await Reservation_1.default.findOne({
        book: bookId,
        status: 'Pending'
...
async function expireReservations() {
    const now = new Date();
    const expiredReservations = await Reservation_1.default.find({
        status: { $in: ['Pending', 'Assigned'] },
        expiresAt: { $lte: now }
    }).populate('user').populate('book');
    const expiredReservationDocs = [];
    for (const reservation of expiredReservations) {
        ...
        const user = reservation.user;
        const book = reservation.book;
        (0, emailService_1.sendReservationExpiredEmail)(user.email, book.title).catch(err => console.error('Không thể gửi email thông báo đặt trước hết hạn:', err));
        // Nếu là reservation Pending, cập nhật queue position cho các reservation khác
        if (wasPending) {
            await updateQueuePositions(bookId);
        }
...
async function sendExpiryReminders() {
    ...
    const expiringReservations = await Reservation_1.default.find({
        status: 'Assigned',
        expiresAt: {
            $gt: now,
            $lte: oneHourFromNow
    ...
    for (const reservation of expiringReservations) {
        const user = reservation.user;
        const book = reservation.book;
        (0, emailService_1.sendReservationExpiringSoonEmail)(user.email, book.title, reservation.expiresAt).catch(err => console.error('Không thể gửi email nhắc nhở đặt trước sắp hết hạn:', err));
    }
}
