...
export const getAdminDashboard = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const [
      ...
      // Mượn mới 30 ngày
      Borrow.countDocuments({ borrowDate: { $gte: last30Days } }),
      // Trả 30 ngày
      Borrow.countDocuments({ 
        status: 'Returned',
    ...
    const [
      ...
      Violation.countDocuments({ createdAt: { $gte: last30Days } }),
      Violation.aggregate([
        {
          $group: {
            _id: '$type',
    ...
    // ========== FORMAT DỮ LIỆU ==========
    const formatAggregateResult = (data: any[]) => {
    ...
    const paymentTypeStats: Record<string, { count: number; total: number }> = {};
