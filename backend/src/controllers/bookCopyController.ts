...
import BookCopy, { IBookCopy } from '../models/BookCopy';
...
export const getAllBookCopies = async (req: AuthRequest, res: Response) => {
  try {
    const { 
      page = 1, 
      limit = 10, 
...
export const getBookCopyById = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
...
export const getBookCopiesByBookId = async (req: AuthRequest, res: Response) => {
  try {
...
export const createBookCopy = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const existingCopy = await BookCopy.findOne({ barcode: normalizedBarcode });
    if (existingCopy) {
      return res.status(400).json({
        success: false,
    ...
    const updateData: any = { $inc: { stock: 1 } };
...
export const createBulkBookCopies = async (req: AuthRequest, res: Response) => {
  try {
  ...
  try {
    ...
    if (status && status !== bookCopy.status) {
      ...
      if (wasAvailable && !willBeAvailable) {
        ...
        await Book.findByIdAndUpdate(bookCopy.bookId, {
          $inc: { available: -1 }
        });
    ...
    if (barcode !== undefined) updateData.barcode = barcode.toUpperCase();
...
export const toggleBookCopyStatus = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
...
export const getBookCopyStatistics = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const statistics = await BookCopy.aggregate([
      { $match: matchQuery },
      {
        $group: {
          _id: '$status',
    ...
    const conditionStats = await BookCopy.aggregate([
      { $match: matchQuery },
      {
        $group: {
          _id: '$condition',
    ...
    const total = await BookCopy.countDocuments(matchQuery);
    const active = await BookCopy.countDocuments({ ...matchQuery, isActive: true });
    ...
    res.status(200).json({
      ...
      data: {
        ...
        statusDistribution: statistics.reduce((acc: any, stat: any) => {
          acc[stat._id] = stat.count;
          return acc;
        }, {}),
        conditionDistribution: conditionStats.reduce((acc: any, stat: any) => {
          acc[stat._id] = stat.count;
          return acc;
        }, {})
      }
...
async function adjustBookAvailabilityOnStatusChange(bookId: any, oldStatus: string, newStatus: string) {
  const wasAvailable = oldStatus === 'available';
  const willBeAvailable = newStatus === 'available';
  if (wasAvailable && !willBeAvailable) {
...
export const markBorrowed = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const bookCopy = await BookCopy.findById(id);
    if (!bookCopy) {
...
export const markDamaged = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const bookCopy = await BookCopy.findById(id);
    if (!bookCopy) {
...
export const markLost = async (req: AuthRequest, res: Response) => {
  try {
    const { id } = req.params;
    const bookCopy = await BookCopy.findById(id);
    if (!bookCopy) {
